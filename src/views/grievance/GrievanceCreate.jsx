...
class GrievanceCreate extends Component {
  ...
  submit = () => {
    ...
    const data={
      name,phone_no,address,subject,content
    };
  ...
  handleBlur = (event) => {
    ...
    switch (name) {
      ...
      case "phone_no":
        ...
        value.length === 0 ? this.setState({ phoneError: "Phone no is required" }) : this.setState({ phoneError: msg });
        break;
      case "address":
        value.length === 0 ? this.setState({ addressError: "Address is required" }) : this.setState({ addressError: "" });
        break;
      case "subject":
        value.length === 0 ? this.setState({ subjectError: "Subject is required" }) : this.setState({ subjectError: "" });
        break;
      case "content":
        break;
      default:
  ...
  render() {
    const { agreed, name, phone_no, address, subject, content, loading } = this.state;
    const { nameError,subjectError,addressError,phoneError,contentError } = this.state;
      ...
      <Grid container spacing={3}>
        <Grid item md={6}>
          <TextField value={name} name={"name"} margin={"dense"} required={true} fullWidth={true}
                     ...
                     onChange={this.changeField.bind(this, "name")} onBlur={this.handleBlur.bind(this)}
        ...
        <Grid item md={6}>
          <TextField value={phone_no} name={"phone_no"} margin={"dense"} required={true} fullWidth={true}
                     ...
                     onBlur={this.handleBlur.bind(this)} onChange={this.changeField.bind(this, "phone_no")}
        ...
        <Grid item md={12}>
          <TextField value={address} name={"address"} margin={"dense"} required={true} fullWidth={true}
                     ...
                     onBlur={this.handleBlur.bind(this)} onChange={this.changeField.bind(this, "address")}
                     error={Boolean(addressError)} helperText={addressError}/>
        </Grid>
        <Grid item md={12}><Divider component={"li"}/></Grid>
        <Grid item md={12}>
          <TextField value={subject} name={"subject"} margin={"dense"} required={true} fullWidth={true}
                     ...
                     onBlur={this.handleBlur.bind(this)} onChange={this.changeField.bind(this, "subject")}
                     error={Boolean(subjectError)} helperText={subjectError}/>
        </Grid>
    ...
    const actions =
      <>
        ...
        <Button href={"#"} name="reset" variant={"outlined"} onClick={this.reset} color={"secondary"}>Reset</Button>
      </>;

    const grievanceFormWrap =
      <Grid item xs={12} sm={12} md={10}>
        <Card>
          <Grid item><CardHeader title={title} subheader={subtitle}/><Divider component={"li"}/></Grid>
          <CardContent>
            {forms}
          </CardContent>
    ...
    return (
      <Grid container justify="flex-start">
        ...
        <SubmitDialog open={this.state.submit} title={"Submit Grievance"} text={"Please wait..."}/>
      </Grid>
    );
  }
}
