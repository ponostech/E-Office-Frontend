...
class ShopRenewApplication extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([self.fetchTrades(), self.fetchDocuments(), self.fetchLocalCouncil(),this.getShopApplication()])
      .finally(function() {
        self.setGlobal({ loading: false });
      });
  }

  getShopApplication=async()=>{
    const { id } = this.props.match;
    this.shopService.get(id,
      errorMsg=>this.setGlobal({errorMsg}),
      shopApplication=>this.setState({shopApplication}))
  }
  ...
  onVerifiedOtp = (verified) => {
    ...
    if (verified) {
      ...
      this.shopService.resubmit(this.state,
        errorMsg => this.setGlobal({ errorMsg }),
        msg => {
          this.setState({
            success: (
...
export default withRouter(withStyles(style)(ShopRenewApplication));