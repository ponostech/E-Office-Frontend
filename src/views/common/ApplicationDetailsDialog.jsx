...
class ApplicationDetailsDialog extends React.Component {
  ...
  checkFieldHistory = value => {
    axios
      .get(`application/${this.props.application.id}/logs`, {
        params: {
      ...
      .then(res => {
        if (res.data)
          this.setState({
            openFieldHistory: true,
            currentFieldDetails: { response: res.data, field: value }
          });
  ...
  render() {
    const { application, open, title, file, onClose } = this.props;
    const {
      openMessage,
      submit,
      submitTitle,
    ...
    let allowed =
      LoginService.getCurrentUser() === null
        ? false
        : LoginService.getCurrentUser().id === file.current_user_id;

    const action = (
      <>
        {allowed && (
          <Button
            onClick={e => this.setState({ openMessage: true })}
            color="primary"
          >
            Send Message
          </Button>
        ...
        {allowed && (
          <Button
            onClick={e => this.setState({ openFine: true })}
            color="primary"
          >
            Impose Fine
          </Button>
    ...
    let leftRow = LEFT_ITEMS.map(val => (
      <DetailViewRow
        checkFieldHistory={this.checkFieldHistory}
        changed={val.changed}
        field={val.field}
        key={val.key}
        primary={val.name}
    ...
    let documents =
      application === null
        ...
        ? application.documents.map(val => (
            <FileViewRow key={val.id} data={val} />
          ))
        : [];
    let additionalDocument =
      application === null
        ...
        ? application.addl_documents.map(val => (
            <FileViewRow key={val.id} data={val} />
          ))
        : [];
    ...
    let content = (
      <Grid container>
        ...
        <Grid item md>
          <img
            style={{ width: 300 }}
            src={application ? application.passport : ""}
            alt="Photograph of Applicant"
          />
          <List subheader={<ListSubheader>Documents</ListSubheader>}>
            <Divider />
            {documents}
          </List>
          <List subheader={<ListSubheader>Additional Document</ListSubheader>}>
            <Divider />
            {additionalDocument}
          </List>
        </Grid>
    ...
    return (
      <>
        ...
        <SendMessageDialog
          ...
          open={openMessage}
          onClose={e => this.setState({ openMessage: false })}
          onMessageSend={this.sendMessage}
        />
        <ImposedFineDialog
          onClose={() => this.setState({ openFine: false })}
          open={openFine}
        ...
        {openFieldHistory ? (
          <ApplicationFieldHistoriesDialog
            data={currentFieldDetails}
            open={openFieldHistory}
            onClose={() => this.setState({ openFieldHistory: false })}
          />
        ) : null}
      </>
    );
