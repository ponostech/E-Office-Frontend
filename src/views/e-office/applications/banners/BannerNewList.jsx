...
class BannerNewList extends Component {
  ...
  processResult = res => {
    if (res.data.status)
      this.setState({ loading: false, banners: res.data.data.banners });
  };
  ...
  openAssignment = data =>
    this.setState({ file: data.file, openAssignment: true });
  ...
  confirmTakeFile = () =>
    axios
      .post(FILE_TAKE(this.state.banner.file.id))
