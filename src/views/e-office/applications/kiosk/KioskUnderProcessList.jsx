...
import { KIOSK_LIST, GET_STAFF, FILE_CALL } from "../../../../config/ApiRoutes";
...
class KioskUnderProcessList extends Component {
  ...
  getData = () => {
    axios
      .get(KIOSK_LIST, { params: { status: "in-process" } })
  ...
  processResult = res => {
    if (res.data.status)
      this.setState({ tableData: res.data.data.kiosk_applications });
  ...
  viewFile = data =>
    this.props.history.push(FILE_DETAIL_ROUTE(data.kiosk.file.id));

  openAssignment = data =>
    this.setState({ file: data.kiosk.file, openAssignment: true });
  ...
  confirmTakeFile = () =>
    axios.post(FILE_CALL(this.state.singleData.kiosk.file.id)).then(() => {
      ...
      },
      {
        ...
        options: {
          customBodyRender: value =>
            value ? value.advertiser.type.toUpperCase() : "NA"
        }
      },
      {
        name: "created_at",
        ...
        options: {
          customBodyRender: value =>
            value ? moment(value).format("Do MMMM YYYY") : "NA"
        }
      },
      {
        name: "kiosk",
        ...
        options: {
          customBodyRender: value =>
            value.file.desk.staff.name +
            " (" +
            value.file.desk.staff.designation +
            ")"
        }
      },
      {
          ...
          customBodyRender: (value, tableMeta) => {
            ...
            return (
              <>
                ...
                >
                  <Icon fontSize="small" className={classes.actionIcon}>
                    pin_drop
                  </Icon>
                </IconButton>
                <IconButton
...
export default withRouter(withStyles(styles)(KioskUnderProcessList));
