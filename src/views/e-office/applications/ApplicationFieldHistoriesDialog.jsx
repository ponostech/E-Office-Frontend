        ...
        >
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Button onClick={this.props.onClose} color="inherit">Close</Button>
            </Toolbar>
          </AppBar>
          <List>
            {this.props.data.response.data.logs.length ? this.props.data.response.data.logs.map((val) =>
              <>
                <ListItem button>
                  <ListItemIcon>
                    <Icon color='primary'>done</Icon>
                  </ListItemIcon>
                  <ListItemText primary={'Changed from "' + val.old_value + '" to "' + val.new_value + '"'}
                                secondary={moment(val.new_date.date).format('Do MMMM YYYY (\dddd\)')} />
                </ListItem>
                <Divider/>
              </>) : <ListItem button><ListItemText secondary='No log found' color='secondary'/></ListItem>
            }
          </List>
          <DialogActions>
