...
class HoardingRateDialog extends Component {
  constructor(props) {
    super(props);
    this.state = {
      type: "Hoarding/Kiosk",
  ...
  invalid = () => {
    const { type, displayType, category } = this.state;
  ...
  render() {
    ...
    const { displayTypeError, categoryError, rateError } = this.state;
    const { displayTypes, categories } = this.state;
    ...
    return (
      ...
      >
        ...
        <DialogActions>
          <Button
            disabled={
              !Boolean(type) || !Boolean(displayType) || !Boolean(category)
            }
            href={"#"}
            variant={"outlined"}
            color={"primary"}
...
export default withStyles(styles)(HoardingRateDialog);
