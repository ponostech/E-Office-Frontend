...
class FileNewList extends Component {
  ...
  getTableData = () =>
    axios.get(ApiRoutes.FILE, { params: { status: "closed" } });
  ...
  closeFileReOpenDialog = () =>
    this.setState({ singleData: [], openFileReOpenDialog: false });

  takeFile = data => this.setState({ singleData: data, openTakeFile: true });

  confirmTakeFile = () =>
    axios
      .post(FILE_TAKE(this.state.singleData.id))
      .then(res => this.confirmTakeFileResponse(res));

  confirmTakeFileResponse = res => {
    if (res.data.status) {
      this.setState({
        successMsg: "File called successfully",
        openTakeFile: false
      });
  ...
  reOpenFile = data =>
    this.setState({ singleData: data, openFileReOpenDialog: true });
  ...
  render() {
              ...
              <>
                ...
                <Tooltip title="Re-Open File">
                  <IconButton
                    ...
                    color="secondary"
                    size="medium"
                    onClick={this.reOpenFile.bind(this, data)}
                  >
    ...
    const files = (
      <CardContent>
        <MUIDataTable
          title={"File: List of Closed Files"}
          data={tableData}
    ...
    return (
      <>
        ...
        {openFileReOpenDialog && (
          <ConfirmDialog
            onCancel={this.closeFileReOpenDialog}
            open={openFileReOpenDialog}
            onConfirm={this.confirmFileReOpen}
