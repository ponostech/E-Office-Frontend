...
class FileDraftLicenseDialog extends Component {
  ...
  getData = () => {
    axios
      .get(GET_LICENSE_TEMPLATE(this.props.module))
      .then(res => {
        if (res.data.status)
          this.setState({
            loading: false,
  ...
  result = (res, fileId) => {
    ...
    setTimeout(() => {
      ...
      this.props.history.push(
        FILE_DETAIL_ROUTE(fileId) + "/view/draft-licenses"
      );
    }, 1000);
  };
  ...
  storeData = () => {
    ...
    axios
      ...
      .catch(err =>
        this.setState({ submit: false, errorMsg: "Network Error" })
      );
  };
