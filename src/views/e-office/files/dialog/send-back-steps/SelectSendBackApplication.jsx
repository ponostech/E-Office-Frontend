...
import ApplicationResolver, {
  getApplicationTitle
} from "../common/ApplicationResolver";
import GridContainer from "../../../../../components/Grid/GridContainer";
import GridItem from "../../../../../components/Grid/GridItem";

class SelectSendBackApplication extends Component {
  constructor(props) {
    super(props);
    this.state = {
      applications: [],
  ...
  componentDidMount() {
    axios
      .get("/files/" + this.props.file.id + "/applications", {
    ...
    return (
      <GridContainer>
        <GridItem xs={12} sm={12} md={6}>
          <CardHeader title={"Please Select Application to Send Back"} />
          <Divider component={"div"} />
          <List>
            {applications.map((application, index) => (
              <DetailViewRow
                key={index}
                click={e => this.selectApplication(application)}
                primary={getApplicationTitle(application).title}
                secondary={getApplicationTitle(application).subtitle}
              >
                <IconButton
                  href={"#"}
                  onClick={e => this.selectApplication(application)}
                >
          ...
          <Button
            ...
            variant={"contained"}
            color={"primary"}
            onClick={e => onSelectApplication(selectedApplication)}
          >
            Next
          </Button>
        </GridItem>
        <GridItem xs={12} sm={12} md={6}>
          {selectedApplication && (
            <List>
              {ApplicationResolver(selectedApplication).map(value => (
                <DetailViewRow primary={value.name} secondary={value.value} />
              ))}
            </List>
          )}
        </GridItem>
...
SelectSendBackApplication.propTypes = {
  file: PropTypes.object.isRequired,
  onSelectApplication: PropTypes.func.isRequired
  // onNext:PropTypes.func.isRequired
};
export default SelectSendBackApplication;
