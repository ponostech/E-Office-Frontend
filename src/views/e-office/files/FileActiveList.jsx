...
import { CardContent, Icon, IconButton } from "@material-ui/core";
...
class FileNewList extends Component {
  ...
  getData = () => {
    axios
      .all([this.getTableData(), this.getStaffs()])
      .then(
        axios.spread((tableData, staffs) =>
          this.processDataResponse(tableData, staffs)
        )
      )
  ...
  getTableData = () =>
    axios.get(ApiRoutes.FILE, { params: { status: "active" } });

  processDataResponse = (tableData, staffs) => {
    if (tableData.data.status && staffs.data.status)
      this.setState({
        tableData: tableData.data.data.files,
  ...
  processSendResponseSuccess = () => {
    this.setState({
      successMsg: "File sent successfully",
      openAssignment: false
    });
  ...
  openAssignment = data =>
    this.setState({ singleData: data, openAssignment: true });

  closeAssignment = () =>
    this.setState({ singleData: [], openAssignment: false });

  takeFile = data => this.setState({ singleData: data, openTakeFile: true });

  confirmTakeFile = () =>
    axios
      .post(FILE_CALL(this.state.singleData.id))
  ...
  archiveFile = data =>
    this.setState({ singleData: data, openFileArchiveDialog: true });
  ...
  closeFile = data =>
    this.setState({ singleData: data, openFileCloseDialog: true });
  ...
  updateStatus = status =>
    axios.post(FILE_STATUS_UPDATE(this.state.singleData.id), {
      status: status
    });
  ...
  render() {
      ...
      },
      {
        ...
        options: {
          customBodyRender: data =>
            data ? data : <span style={{ color: "red" }}>New File</span>
        }
      },
      {
        name: "created_at",
              ...
              <>
                ...
                <Tooltip title="Archive File">
                  <IconButton
                    ...
                    color="secondary"
                    size="medium"
                    onClick={this.archiveFile.bind(this, data)}
                  >
                ...
                <Tooltip title="Close File">
                  <IconButton
                    ...
                    color="secondary"
                    size="medium"
                    onClick={this.closeFile.bind(this, data)}
                  >
    ...
    const files = (
      <CardContent>
        <MUIDataTable
          title={"File: List of Active Files"}
          data={tableData}
      ...
      <>
        ...
        {openFileCloseDialog && (
          <ConfirmDialog
            ...
            onConfirm={this.confirmedCloseFile}
        ...
        {openFileArchiveDialog && (
          <ConfirmDialog
            ...
            onConfirm={this.confirmedArchiveFile}
