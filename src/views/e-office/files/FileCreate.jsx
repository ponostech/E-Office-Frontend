...
class FileCreate extends Component {
  ...
  getFiles = () =>
    axios
      .get(ApiRoutes.FILE, { params: { status: "all" } })
      .then(res => {
        let temp = [];
        if (res.data.status) {
  ...
  getBranch = () => {
    this.setGlobal({ loading: true });
    axios
      .get("setting/branches")
      .then(res => {
        if (res.data.status)
          this.setState({ branches: res.data.data.branches });
        else this.setGlobal({ errorMsg: res.data.messages });
      })
      .catch(err => this.setGlobal({ errorMsg: err.toString() }))
      .then(() => this.setGlobal({ loading: false }));
  ...
  getClassifications = () => {
    this.setGlobal({ loading: true });
    axios
      .get("setting/classifications")
      .then(res => {
        if (res.data.status)
          this.setState({ classifications: res.data.data.classifications });
        else this.setGlobal({ errorMsg: res.data.messages });
      })
      .catch(err => this.setGlobal({ errorMsg: err.toString() }))
      .then(() => this.setGlobal({ loading: false }));
  ...
  successGroup = ({ data }) =>
    this.setState({ groupHeadOptions: data.group_heads });

  successMain = ({ data }) =>
    this.setState({ mainHeadOptions: data.main_heads });
  ...
  handleBlur = (identifier, value) => {
    switch (identifier) {
      case "groupHead":
        !Boolean(this.state.groupHead)
          ? this.setState({ groupHeadError: "Group head is required" })
          : this.setState({ groupHeadError: "" });
        break;
      case "mainHead":
        !Boolean(this.state.mainHead)
          ? this.setState({ mainHeadError: "Main head is required" })
          : this.setState({ mainHeadError: "" });
        break;
      case "subHead":
        !Boolean(this.state.subHead)
          ? this.setState({ subHeadError: "Sub head is required" })
          : this.setState({ subHeadError: "" });
        break;
      case "subject":
        !Boolean(this.state.subject)
          ? this.setState({ subjectError: "Group head is required" })
          : this.setState({ subjectError: "" });
        break;
      case "branch":
        !Boolean(this.state.branch)
    ...
    switch (name) {
      case "groupHead":
        Boolean(value)
          ...
          : this.setState({ mainHeadOptions: [] });
        break;
      case "mainHead":
        Boolean(value)
          ? this.getSub(value.id)
          : this.setState({ subHeadOptions: [] });
        break;
      default:
        break;
    }
    ...
    return (
      <GridContainer justify={"flex-start"}>
        <GridItem xs={12} sm={12} md={6}>
          <Card>
            <CardHeader
              title={NewFileViewModel.TITLE}
              subheader={NewFileViewModel.SUBTITLE}
            />
            <CardContent>
              <Grid container xs={12} spacing={2}>
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    variant={"outlined"}
                    onChange={event =>
                      this.handleChange("subject", event.target.value)
                    }
            ...
            <CardActions style={{ justifyContent: "flex-end" }}>
              <Button
                ...
                variant="outlined"
                color="primary"
                onClick={this.handleClick.bind("submit", this)}
              >
                Create
              </Button>{" "}
              <Button
