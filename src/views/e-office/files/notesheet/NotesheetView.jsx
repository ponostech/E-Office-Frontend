...
class NotesheetView extends Component {
  ...
  processResponse = res =>
    res.data.status && this.formatNote(res.data.data.notesheets);
  ...
  formatNote = note => {
    let formattedNote = note.map(data => {
      ...
      temp["footerDesignation"] = data.staff
        ? data.staff.staff.designation
        : "";
      temp["footerTitle"] =
  ...
  openAttachments = attachments =>
    this.setState({ openAttachments: true, attachments });
  ...
  render() {
    ...
    if (!loading)
      if (this.state.note.length > 0)
        noteList = (
          <Timeline
            openAttachment={this.openAttachments}
            simple
        ...
        noteList = (
          <div style={{ padding: 20 }}>New File. Note not available.</div>
        );

    let allowed =
      LoginService.getCurrentUser().id === this.props.file.current_user_id;
    return (
      <>
        <CardHeader
