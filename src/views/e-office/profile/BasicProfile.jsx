...
import { Card, CardActions, CardContent, Button,CardHeader, InputAdornment, TextField } from "@material-ui/core";
...
class BasicProfile extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([this.fetchBranches(), this.fetchRoles()])
  ...
  getUser=async ()=>{
    ...
    await this.staffService.getStaff(id,errorMsg=>this.setGlobal({errorMsg}),staff=>this.setStaff(staff))
  }

  fetchBranches = async () => {
    await this.staffService.getBranch(errorMsg => this.setGlobal({ errorMsg }),
      branches => this.setState({ branches }));
  };

  fetchRoles = async () => {
    await this.staffService.getRoles(errorMsg => this.setGlobal({ errorMsg }),
      roles => this.setState({ roles }));
  };

  updateProfile = () => {
    const {id,photo, name, email, phone, designation, address, dob, branch, role, bloodGroup } = this.state;
    this.setState({submit:true})
    this.staffService.update({
      id,photo,name,email,phone_no:phone,designation,address,dob:moment(dob).format("YYYY/MM/DD"),branch:branch.value,role:role.value,bloodGroup
  ...
  onBlur = (name, value) => {
    switch (name) {
      case "name":
        value.length <= 0 ? this.setState({ nameError: "Name is required" }) : this.setState({ nameError: "" });
        break;
      case "email":
        if (value.length <= 0)
      ...
      case "address":
        ...
        break;
      case "designation":
        value.length <= 0 ? this.setState({ designationError: "Designation is required" }) : this.setState({ designationError: "" });
        break;
      case "dob":
        value.length <= 0 ? this.setState({ dobError: "Date of Birth is required" }) : this.setState({ dobError: "" });
        break;
      case "branch":
        value.length <= 0 ? this.setState({ branchError: "Branch is required" }) : this.setState({ branchError: "" });
        break;
      case "role":
        value.length <= 0 ? this.setState({ roleError: "Role is required" }) : this.setState({ roleError: "" });
        break;
  ...
  render() {
    ...
    const { name, photo,email, phone, designation, address, dob, branch, role, bloodGroup,submit } = this.state;
    const { nameError, emailError, phoneError, designationError, addressError, dobError, branchError, roleError, bloodGroupError } = this.state;
    return (
      <Card>
        <CardHeader style={{textAlign:"center"}} title={<AvatarView url={photo} onChange={(photo)=>this.setState({photo})}/>}/>
        <CardContent>
        ...
        <CardActions>
          <Button onClick={e => this.updateProfile()} color={"primary"} variant={"outlined"}>Update</Button>
          <Button onClick={e => this.reset()} color={"secondary"} variant={"outlined"}>Reset</Button>
        </CardActions>

        <SubmitDialog open={submit} title={"Updating Profile"} text={"Please wait ... "} />
      </Card>
    );
  }
}
